...
int auth_UDP_Receiver(uint8_t *recv_data)
{
    ...
    if (recv_len > 0 && memcmp(&clntaddr.sin_addr, &serv_addr.sin_addr, 4) == 0 && ((recv_data[0] == 0x07) || ((recv_data[0] == 0x4d) && (recv_data[1] == 0x38))))
    {
...
int auth_8021x_Receiver(uint8_t *recv_data)
{
    ...
    // 过滤掉非0x888e的报文
    if (recv_len > 0 && (0 == memcmp(recv_hdr->h_dest, local_ethhdr->h_source, ETH_ALEN)) && (htons(ETH_P_PAE) == recv_hdr->h_proto))
    {
...
int Authentication(int client)
{
    ...
    // while (ret = getIfIP(auth_8021x_sock) != 0)
    //     sleep(1);
    printIfInfo();
